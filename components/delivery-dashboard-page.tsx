...
export function DeliveryDashboardPage({ currentRole, userName }: DeliveryDashboardPageProps) {
  const [selectedStopId, setSelectedStopId] = useState(deliveryStops[0]?.id ?? "");

  const selectedStop = useMemo(
    () => deliveryStops.find((stop) => stop.id === selectedStopId) ?? deliveryStops[0],
    [selectedStopId]
  );
                  ...
                  >
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="font-semibold text-brand-ink">{stop.customer}</p>
                          <p className="text-sm text-brand-muted">{stop.orderId}</p>
                        </div>
                        <span className="rounded-full bg-brand-orange px-3 py-1 text-[10px] font-bold text-brand-ink">{stop.distanceKm} KM</span>
                      </div>
                      <p className="mt-3 text-[11px] text-brand-muted">ETA: {stop.eta}</p>
                    </button>
                    <Link
                      href={`/delivery/complete/${stop.id}`}
            ...
            <div className="grid lg:grid-cols-2 lg:divide-x lg:divide-brand-border/30">
              <div className="space-y-8 p-6 lg:p-8">
                <div>
                  ...
                  <p className="mt-2 text-[3rem] font-bold tracking-tight text-brand-green">{selectedStop.orderId}</p>
                </div>
              ...
              <div className="space-y-8 bg-brand-panel-soft/40 p-6 lg:p-8">
                <div>
                  ...
                  <div className="mt-4 rounded-xl border border-brand-border/50 bg-white p-4">
                    <p className="font-semibold text-brand-ink">{selectedStop.address}</p>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <a href={`tel:${selectedStop.phone.replace(/\s+/g, "")}`} className="inline-flex items-center justify-center gap-3 rounded-xl border border-brand-border bg-white px-5 py-4 text-base font-bold text-brand-ink">
                    <Phone className="h-5 w-5 text-brand-green" />
                    Call
                  </a>
                  <a href={selectedStop.mapUrl} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-3 rounded-xl border border-brand-border bg-white px-5 py-4 text-base font-bold text-brand-ink">
                ...
                <Link
                  href={`/delivery/complete/${selectedStop.id}`}
