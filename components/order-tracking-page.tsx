...
import { CheckCircle2, Eye, MapPin, MessageCircleMore } from "lucide-react";
...
export function OrderTrackingPage({ orderId, session }: OrderTrackingPageProps) {
  return (
    <div className="app-shell pb-12">
      ...
      <section className="mx-auto max-w-5xl px-4 py-10 lg:px-6">
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="mt-8 text-[3rem] font-bold tracking-tight text-brand-green">Order Submitted Successfully!</h1>
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-[1.65fr_0.95fr]">
          <div className="soft-card rounded-xl p-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <span className="status-pill bg-brand-orange text-white">Pending Payment Verification (Manual)</span>
            </div>
            <h2 className="mt-4 text-[2.1rem] font-bold tracking-tight text-brand-ink">{orderId}</h2>
            <p className="mt-1 text-sm text-brand-muted">Placed today and queued for manual verification.</p>
          ...
          <div className="rounded-xl bg-brand-panel-alt p-6">
            ...
            <div className="mt-5 space-y-4 text-sm text-brand-ink">
              <div className="flex items-center justify-between"><span>Items (4)</span><span>₹ 1,240.00</span></div>
              <div className="flex items-center justify-between"><span>Delivery Fee</span><span className="font-bold text-brand-green">FREE</span></div>
              <div className="flex items-center justify-between border-t border-brand-border/60 pt-4"><span className="font-bold">Total Amount</span><span className="text-[1.5rem] font-bold text-brand-orange-deep">₹ 1,240.00</span></div>
            </div>
            <div className="mt-5 flex items-start gap-3 rounded-lg border border-brand-border/50 bg-brand-panel px-4 py-4">
              ...
              <div>
                ...
                <p className="text-sm text-brand-muted">123 Green Lane, Sector 4, New Delhi</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 flex flex-col items-center justify-center gap-4 md:flex-row">
          <Link
            href={`https://wa.me/919876521000?text=${encodeURIComponent(`Hi Bhagwandas Traders, I need help with order ${orderId}.`)}`}
          ...
          <button type="button" className="inline-flex items-center justify-center gap-3 rounded-xl bg-brand-green px-6 py-4 text-base font-bold text-white transition hover:brightness-110">
        ...
        <div className="mt-8 overflow-hidden rounded-2xl shadow-focus">
          <div className="relative h-64 bg-gradient-to-br from-brand-panel-alt via-brand-green-fixed/30 to-brand-orange/20">
            ...
            <div className="absolute bottom-6 left-6">
              <p className="text-2xl font-semibold text-brand-ink">Preparing the freshest produce for you</p>
            </div>
          </div>
        </div>
      </section>
