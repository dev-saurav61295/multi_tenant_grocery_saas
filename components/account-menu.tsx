...
export function AccountMenu({ session }: AccountMenuProps) {
  if (!session) {
    return (
      <Link href="/login" className="text-sm font-semibold text-brand-green hover:text-brand-orange-deep">
        Login
      </Link>
    );
  }
  ...
  return (
    <form action={logout} className="flex items-center gap-3 text-sm font-semibold text-brand-ink">
